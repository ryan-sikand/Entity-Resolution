...
export const Sidebar = ({ activeView, onViewChange, highRiskCount, inProgressCount, completedCount, searchQuery, onSearchChange, onFilterClick }: SidebarProps) => {
  ...
  return (
    <div className="fixed left-0 top-0 flex h-screen w-[176px] flex-col border-r border-gray-800 bg-[#1a1d29] xl:w-[200px]">
      {/* Logo */}
      <div className="flex items-center gap-2 border-b border-gray-800 p-3 xl:p-4">
        <div className="w-8 h-8 bg-red-500 rounded flex items-center justify-center">
          <span className="text-white font-bold text-sm">I</span>
        </div>
        <span className="truncate text-sm font-semibold text-white xl:text-base">Investigations</span>
      </div>

      {/* Search */}
      <div className="border-b border-gray-800 p-2.5 xl:p-3">
      ...
      <nav className="flex-1 p-2 space-y-1">
        {navItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center justify-between rounded-md px-2.5 py-2 text-sm transition-colors xl:px-3 ${
              activeView === item.id
      ...
      {/* User Profile */}
      <div className="border-t border-gray-800 p-2.5 xl:p-3">
